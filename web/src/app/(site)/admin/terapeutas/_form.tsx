...
function SubmitButton({ loading }: { loading: boolean }) {
  return (
    ...
    >
      {loading ? "Salvando..." : "Adicionar Terapeuta"}
    </button>
  )
}
    ...
    <form onSubmit={onSubmit} className="border border-slate-200 p-6 rounded-sm mb-10">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <FormField label="Nome">
          <input value={name} onChange={e => setName(e.target.value)} className="w-full border border-slate-200 p-3 rounded-sm" placeholder="Ex: Beatriz" />
        </FormField>
        ...
        <FormField label="Telefone (WhatsApp)">
          <input value={phone} onChange={e => setPhone(e.target.value)} className="w-full border border-slate-200 p-3 rounded-sm" placeholder="351962252659" />
        </FormField>
        ...
        <FormField label="Descrição / Bio">
          <textarea value={bio} onChange={e => setBio(e.target.value)} className="w-full border border-slate-200 p-3 rounded-sm min-h-32" placeholder="Sobre a terapeuta" />
        </FormField>
